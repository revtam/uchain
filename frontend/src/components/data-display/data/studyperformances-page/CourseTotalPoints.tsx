...
export interface CourseTotalPointsProps {
    courseId: string;
    studentId?: string;
}

const CourseTotalPoints: React.FunctionComponent<CourseTotalPointsProps> = ({
    courseId,
    studentId,
}: CourseTotalPointsProps) => {
    ...
    useEffect(() => {
        (async () => {
            if (performanceControllerContract) {
                let pointsFetchMethod = () => performanceControllerContract.calculatePoints(courseId);
                if (studentId)
                    pointsFetchMethod = () =>
                        performanceControllerContract.calculatePointsOfStudent(studentId, courseId);
