...
import { Course, Grading } from "../../../../utils/converter/internal-types/internalTypes";
...
export interface GradingDataProps {
    course: Course;
    studentId?: string;
    editEnabled?: boolean;
}

const GradingData: React.FunctionComponent<GradingDataProps> = ({
    course,
    studentId,
    editEnabled = false,
}: GradingDataProps) => {
    ...
    useEffect(() => {
        if (performanceViewContract) {
            ...
            alertError(gradeFetchMethod, setErrorMessage)
                .then((grading) => convertToGradingInternal(grading))
